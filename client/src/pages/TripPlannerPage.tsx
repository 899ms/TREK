...
import DayImpactList from '../components/shared/DayImpactList'
...
function TripPlannerPageDesktop(): React.ReactElement | null {
  ...
  const {
    ...
    deleteDayId, setDeleteDayId, deleteDayTitle, deleteDayLines, handleDeleteDay, confirmDeleteDay,
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', ...fontStyle }}>
      ...
      />
      {/* Deleting a day from the reorder dialog: the question lists what goes
          with the day, and a booked stay in it shows in the danger tone. */}
      <ConfirmDialog
        isOpen={deleteDayId != null}
        onClose={() => setDeleteDayId(null)}
        onConfirm={confirmDeleteDay}
        title={deleteDayTitle}
        message={t('dayplan.deleteDayBody')}
        confirmLabel={t('dayplan.deleteDay')}
      >
        <DayImpactList lines={deleteDayLines} label={deleteDayTitle} />
      </ConfirmDialog>
      <ConfirmDialog
        isOpen={!!stayRelease}
