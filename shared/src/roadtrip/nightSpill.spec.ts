...
describe('spillChains', () => {
  ...
  describe('a night booked into the next morning holds that morning', () => {
    // Check in on day 1 at 08:00, check out on day 2 at 08:00. Until the room is
    // handed back the traveller is at the hotel, so day 2 cannot quietly start at
    // 07:00 somewhere else as if the night had ended at midnight.
    const hotel = () => stop({
      assignmentId: 1, name: 'Mercure Hamburg', checkInTime: '08:00',
      checkoutAt: 2 * 1440 + 480,
    });

    it('FE-NIGHTSPILL-013: a stop pinned before check-out keeps its clock and is flagged late', () => {
      const plan = [
        day(1, 1, [hotel()]),
        day(2, 2, [stop({ assignmentId: 2, name: 'Hotel Panorama', time: '07:00' })]),
      ];

      const chains = spillChains(plan, [], everyLeg(11));
      const dayTwo = chains.find((c) => c.dayNumber === 2)!;

      // The hour the traveller chose is never moved.
      expect(dayTwo.schedule.entries[0]!.arrival).toBe('07:00');
      // But it cannot be reached: the room goes back an hour after it.
      expect(dayTwo.schedule.warnings).toContainEqual({ index: 0, code: 'late', minutes: 60 });
    });

    it('FE-NIGHTSPILL-014: a stop with no clock of its own starts at check-out, not at nothing', () => {
      const plan = [
        day(1, 1, [hotel()]),
        day(2, 2, [stop({ assignmentId: 2, name: 'Bergedorf' })]),
      ];

      const chains = spillChains(plan, [], everyLeg(11));

      expect(chains.find((c) => c.dayNumber === 2)!.schedule.entries[0]!.arrival).toBe('08:00');
    });

    it('FE-NIGHTSPILL-015: the days between a longer stay are left alone', () => {
      // Monday to Thursday: you sleep there, you do not sit there. Only the day the
      // check-out falls in is held.
      const plan = [
        day(1, 1, [stop({ assignmentId: 1, name: 'Mercure Hamburg', checkInTime: '08:00', checkoutAt: 4 * 1440 + 480 })]),
        day(2, 2, [stop({ assignmentId: 2, name: 'Speicherstadt' })]),
        day(4, 4, [stop({ assignmentId: 3, name: 'Bergedorf' })]),
      ];

      const chains = spillChains(plan, [], everyLeg(11));

      expect(chains.find((c) => c.dayNumber === 2)!.schedule.entries[0]!.arrival).toBeNull();
      expect(chains.find((c) => c.dayNumber === 4)!.schedule.entries[0]!.arrival).toBe('08:00');
    });

    it('FE-NIGHTSPILL-016: a check-out on the same day it started binds nothing', () => {
      // A day room, or a stay whose end day is the day it started: the stops after it
      // already follow through the leg chain and need no floor of their own.
      const plan = [
        day(1, 1, [stop({ assignmentId: 1, name: 'Day room', checkInTime: '08:00', checkoutAt: 1 * 1440 + 480 })]),
        day(2, 2, [stop({ assignmentId: 2, name: 'Bergedorf' })]),
      ];
